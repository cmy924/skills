...
| 模式 | 说明 | 适用场景 | 依赖 |
|------|------|---------|------|
| **AI 模式**（默认） | 使用 rembg AI 模型自动抠图 | 复杂背景、渐变、阴影 | `rembg[cpu]`（自动安装） |
| **颜色模式** | 基于背景色 flood-fill 替换 | 纯色背景（白底、单色底） | `Pillow`（自动安装） |
...
脚本位置：`{skill_dir}/scripts/remove-bg.py`

#### AI 模式（推荐，效果最好）

```bash
python "{skill_dir}/scripts/remove-bg.py" "输入图片.png"
```

#### 颜色模式（纯色背景，更快）

```bash
python "{skill_dir}/scripts/remove-bg.py" "输入图片.png" --mode color
```

#### 完整参数

```bash
python "{skill_dir}/scripts/remove-bg.py" <input...> [options]
...
```bash
# 素材库中的图片去白底
python remove-bg.py "素材库/download/红豆.png" -o "素材库/download/红豆_transparent.png"
```
...
python remove-bg.py "素材库/download/*.png" -o "素材库/download/transparent/"
```
...
```bash
# 颜色模式，自动检测背景色
python remove-bg.py icon.png --mode color --tolerance 25

# 指定背景色为纯白
python remove-bg.py icon.png --mode color --bg-color "#FFFFFF" --tolerance 20
```

### 场景 4：边缘有锯齿，加羽化

```bash
python remove-bg.py sprite.png --feather 2
```
...
1. **AI 模式首次使用**会自动下载模型（约 170MB），后续使用直接调用
2. **输出格式**固定为 PNG（支持透明通道），即使输入是 JPG
3. **颜色模式**使用 flood-fill 算法从边缘开始去除，不会误删图片内部相同颜色的区域
4. **依赖自动安装**：脚本会自动检测并安装 `Pillow` 和 `rembg`，无需手动操作
5. 如果 `rembg` 安装失败（网络问题等），会自动回退到颜色模式

## 故障排除

| 问题 | 原因 | 解决方案 |
|------|------|---------|
| rembg 安装失败 | 网络/pip 问题 | 使用 `--mode color` 回退 |
