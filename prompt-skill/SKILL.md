...
- 优化已有 prompt 以提升出图质量
- 为 `assets.json` 中 prompt 为空的条目补齐提示词
- 确保同一关卡/场景的素材风格一致

## 核心流程

1. **收集上下文** — 读取 `index.tsx`（游戏逻辑）、`素材库/assets.json`（素材清单）、`素材库/素材提示词.md`（已有参考 prompt）
2. **分析素材需求** — 确定每个素材的类别、用途、尺寸、背景要求
3. **生成 prompt** — 按规范输出英文提示词
4. **写入目标文件** — 更新 `assets.json` 或输出到 `素材库/素材提示词.md`
...
- [ ] **尺寸匹配**：与 `assets.json` 中的 `size` 字段对应
...
| 下游 Skill | 消费内容 |
|-----------|---------|
| asset-generate | 读取 `assets.json` 中的 prompt 进行批量生图 |
