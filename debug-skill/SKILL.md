---
name: debug-skill
description: 为 React 小游戏/课件组件添加开发调试面板（debug 模式）。通过 URL 参数 dev=1 激活，支持关卡跳转、步骤跳转、状态监控，面板可拖拽移动、可缩放且内容等比放大。触发词：debug模式、调试面板、dev模式、开发模式、debug panel、调试工具、跳关、跳步骤。
---
...
```tsx
const devJump = (level: LevelId, step: Step) => {
  // 1. 清理所有定时器
      ...
      {devPanelOpen && (
        <div className={styles.devBody}>
          ...
          <div className={styles.devSection}>
            ...
            <div className={styles.devBtnGroup}>
              {levels.map(lv => (
                <button
                  key={lv.id}
          ...
          <div className={styles.devState} style={{ fontSize: `${baseFontSize * 0.83}px` }}>
            Level: <span className={styles.devStateVal}>{currentLevel}</span> |
            Step: <span className={styles.devStateVal}>{currentStep}</span>
          </div>
          {/* 缩放手柄 */}
