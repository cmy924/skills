...
> - **type 11**：text 是提示词字符串，image 是原图文件（multipart/form-data 上传）
...
- `image`：原图文件（通过 multipart/form-data 上传）
...
bun run <skill-directory>/scripts/asset-generate.js --text "Make it smile, keep same style" --image ./assets/character.png --type 11 [options]

# type 12（文生图·数组）
bun run <skill-directory>/scripts/asset-generate.js --text '[{"name":"勺子","prompt":"A cute cartoon spoon","size":"512x512"}]' --type 12 [options]

# type 21（文生语音·数组）
bun run <skill-directory>/scripts/asset-generate.js --text '[{"name":"L1_S1_01","content":"你好","model":10139,"ref_audio":10311,"role_id":"xx"}]' --type 21 [options]
```

| 参数 | 说明 |
|------|------|
| `--text, -t` | (必填) type 11: 提示词字符串; type 12: JSON 对象数组; type 21: JSON 对象数组 `[{name,content,model,ref_audio,role_id}]` |
| `--type` | (必填) 11=图生图, 12=文生图, 21=文生语音 |
| `--image` | (type 11 必填) 原图文件路径，通过 multipart/form-data 上传 |
