...
| 模式 | 用途 | 入口 |
|------|------|------|
| **Pipeline 批量** | 读取 `assets.json` 批量生产图片+TTS | `asset-pipeline.js` |
...
```bash
bun --env-file=<project-root>/.claude/.env run <skill-directory>/scripts/asset-pipeline.js --input ./素材库/assets.json --output ./asset-urls.ts [options]
```
...
```bash
# 批量生成全部素材
bun --env-file=<project-root>/.claude/.env run <skill-directory>/scripts/asset-pipeline.js -i ./素材库/assets.json -o ./asset-urls.ts

# 预览计划
bun --env-file=<project-root>/.claude/.env run <skill-directory>/scripts/asset-pipeline.js -i ./素材库/assets.json -o ./asset-urls.ts --dry-run

# 只补充新增
bun --env-file=<project-root>/.claude/.env run <skill-directory>/scripts/asset-pipeline.js -i ./素材库/assets.json -o ./asset-urls.ts --skip-existing
```
...
1. `extract-skill` 提炼 → 得到 `materials.json` + `tts.json`
...
| 上游 Skill | 提供数据 |
|-----------|---------|
| extract-skill | `materials.json`（素材清单）、`tts.json`（语音清单） |
