...
| 执行顺序 | Skill 名称 | 职责 | 说明 |
|---------|-----------|------|------|
| 1 | `roles-skill` | 查询角色资源库 | 从角色库查询可用角色（小安/小瑞/小布），将 idle/speaking 图片 URL、主题色写入项目角色配置 |
| 2 | `copilot-instructions-skill` | 生成 `.github/copilot-instructions.md` | 分析源码，提取游戏规则/架构/实体等，生成协作指南 |
| 3 | `debug-skill` | 添加 Debug 调试面板 | 为组件添加开发调试面板（URL 参数 `?dev=1` 或路径含 `/dev/` 激活） |
| 4 | `ai-sounds` | 添加交互音效 | 使用 ai-sounds 库为游戏动作添加音效反馈（正确/错误点击、通关、按钮等） |
| 5 | `bgm-skill` | 添加背景音乐 | 提炼 BGM 配置并应用默认模版，实现场景切换与播放策略 |
| 6 | `check-skill` | 规则验证 | 逐条验证游戏规则、状态机、计时器、音频系统等，确保初始化后项目完整正确 |
| 7 | `agent-creator` | 生成开发 Agent | 基于项目源码生成 `.github/agents/game-dev.md`（Copilot Agent，通过 `@game-dev` 调用） |
...
**Step 2 — 执行 copilot-instructions-skill：**
...
```
✅ 初始化组执行完成：
  1. roles-skill — ✅ 已引入角色资源（小安/小瑞/小布）
  2. copilot-instructions-skill — ✅ 已生成 .github/copilot-instructions.md
  3. debug-skill — ✅ 已添加调试面板
  4. ai-sounds — ✅ 已添加交互音效
  5. bgm-skill — ✅ 已添加背景音乐
  6. check-skill — ✅ 规则验证通过
  7. agent-creator — ✅ 已生成 .github/agents/game-dev.md（@game-dev）
```
...
```
⚠️ 初始化组执行完成（有异常）：
  1. roles-skill — ✅ 已引入角色资源
  2. copilot-instructions-skill — ✅ 已生成
  3. debug-skill — ❌ 失败：[具体原因]
  4. ai-sounds — ✅ 已添加交互音效
  5. bgm-skill — ✅ 已添加背景音乐
  6. check-skill — ⚠️ 发现 N 项问题并已修复
  7. agent-creator — ✅ 已生成 @game-dev Agent
```
...
| 执行顺序 | Skill 名称 | 职责 | 说明 |
|---------|-----------|------|------|
| 1 | `extract-skill` | 资源提炼（TTS + 素材） | 提炼 TTS 语音、素材清单，输出 tts.json + materials.json + preview.html |
| 2 | `preview-skill` | AI素材工坊技能（原素材预览） | 打开AI素材工坊页，查看生产线仪表盘，按分类一键触发生产 |
...
> **💡 推荐工作方式：** 执行完 extract-skill 后，执行 preview-skill 打开 `素材库/preview.html`（AI素材工坊页），顶部有三个分类环形进度卡片（场景素材 / 道具+透明化 / TTS），点击卡片中的生产按钮会自动复制包含完整流程的 Copilot 指令到剪贴板。也可以按区块单独点击每个素材类别的生产按钮。
...
2. 分析游戏源码，提炼 TTS 语音数据和素材清单
3. 输出 `素材库/tts.json` + `素材库/materials.json` + `素材库/preview.html`（含生产线仪表盘）

**Step 2 — 执行 preview-skill（AI素材工坊技能）：**

1. 读取 `.claude/skills/preview-skill/SKILL.md` 获取完整指令
2. 在浏览器中打开 `素材库/preview.html`
3. 查看素材生产线仪表盘，确认各分类完成状态
4. 根据需要点击分类生产按钮
...
- **场景类素材**（直接生产）：
  ...
  3. 更新 `materials.json`、`preview.html`、`asset-urls.ts`
