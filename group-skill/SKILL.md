...
| 执行顺序 | Skill 名称 | 职责 | 说明 |
|---------|-----------|------|------|
| 1 | `extract-skill` | 资源提炼（TTS + 素材） | 提炼 TTS 语音、素材清单，输出 tts.json + materials.json |
...
> **💡 推荐工作方式：** 执行完 extract-skill 后，执行 preview-skill（运行 `node .claude/skills/preview-skill/scripts/generate-preview.cjs --open`）生成并打开 `素材库/preview.html`（AI素材工坊页），顶部有三个分类环形进度卡片（场景素材 / 道具+透明化 / TTS），点击卡片中的生产按钮会自动复制包含完整流程的 Copilot 指令到剪贴板。也可以按区块单独点击每个素材类别的生产按钮。
...
3. 输出 `素材库/tts.json` + `素材库/materials.json`
...
- **场景类素材**（直接生产）：
  ...
  3. 更新 `materials.json`、`asset-urls.ts`，重新运行 `preview-skill` 生成预览页
...
- **道具类素材**（生产 + 透明化全流程）：
  ...
  6. 更新 `materials.json`、`asset-urls.ts`，重新运行 `preview-skill` 生成预览页

- **TTS 语音**：
  1. 从 `素材库/assets.json` 提取 TTS 数据
  2. 使用 produce-skill 的 `asset-generate.js --type 21` 生产
  3. 更新 `tts.json`，重新运行 `preview-skill` 生成预览页
