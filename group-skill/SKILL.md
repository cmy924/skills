...
通过提炼素材和 TTS 数据，让提示词技能生成对应的提示词，分别调用素材生产技能应用到游戏中，最后检查整个游戏。
...
| 2 | `tts-skill` | 提炼 TTS 语音 | 提炼角色台词、旁白引导语等 TTS 数据，输出语音规划 JSON |
| 3 | `material-skill` | 提炼素材规划 | 提炼场景、道具、美术资源清单，输出素材规划 JSON |
| 4 | `prompt-skill` | 生成文生图提示词 | 基于素材清单为每个素材生成高质量英文绘图 prompt |
| 5 | `asset-generate` | 素材生产 | 调用 AI Hub 素材生产 API，批量生成图片和 TTS 语音素材 |
| 6 | `check-skill` | 规则验证 | 逐条验证游戏规则、状态机、素材完整性等，确保整体正确 |
...
1. 读取 `.claude/skills/tts-skill/SKILL.md` 获取完整指令
2. 分析游戏源码，提炼角色台词、旁白引导语、UI 语音等 TTS 数据
3. 输出 TTS 语音规划 JSON，未生成的 URL 留空

**Step 3 — 执行 material-skill：**

1. 读取 `.claude/skills/material-skill/SKILL.md` 获取完整指令
2. 分析游戏关卡配置和设计文档，提炼场景、道具等素材清单
3. 输出素材规划 JSON

**Step 4 — 执行 prompt-skill：**

1. 读取 `.claude/skills/prompt-skill/SKILL.md` 获取完整指令
2. 基于 Step 2 和 Step 3 输出的素材清单，为每个素材生成文生图提示词
3. 将提示词写入素材清单对应字段

**Step 5 — 执行 asset-generate：**

1. 读取 `.claude/skills/asset-generate/SKILL.md` 获取完整指令
2. 基于完善好提示词的素材清单和 TTS 数据，调用素材生产 API 批量生成图片和 TTS
3. 更新 `asset-urls.ts` 和 `asset-urls.json`，将素材应用到游戏中
