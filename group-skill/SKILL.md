...
| 2 | `produce-skill` | 一站式素材生产 | 为素材生成 prompt，调用 API 批量生产图片和 TTS，输出 asset-urls.ts |
| 3 | `check-skill` | 规则验证 | 逐条验证游戏规则、状态机、素材完整性等，确保整体正确 |
...
3. 输出 `素材库/tts.json` + `素材库/materials.json` + `素材库/preview.html`

**Step 2 — 执行 produce-skill：**

1. 读取 `.claude/skills/produce-skill/SKILL.md` 获取完整指令
2. 基于 Step 1 输出的素材清单和 TTS 数据，为每个素材生成文生图提示词
