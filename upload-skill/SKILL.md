...
```bash
# 上传单个文件
bun run <skill-directory>/scripts/upload.js ./素材库/download/西红柿.jpg

# 上传整个目录（自动过滤图片/音频文件）
bun run <skill-directory>/scripts/upload.js ./素材库/download/

# 上传并自动更新 asset-urls.ts
bun run <skill-directory>/scripts/upload.js ./素材库/download/ --update-urls

# 输出 JSON 格式结果
bun run <skill-directory>/scripts/upload.js ./素材库/download/西红柿.jpg --json

# 带素材名称映射
bun run <skill-directory>/scripts/upload.js ./素材库/download/ --update-urls --name-map '{"西红柿":"西红柿","spoon":"勺子"}'
```
...
bun run <skill-directory>/scripts/upload.js ./素材库/download/ --update-urls --name-map '{"厨房场景":"厨房场景背景"}'
```
